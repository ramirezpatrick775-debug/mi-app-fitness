    ...
    if (esPremium) {
        ...
        contenidoFinalRutina = `
            ...
            <div id="bloque-premium-web" style="background: #f8d7da; border: 1px solid #f5c6cb; padding: 15px; border-radius: 6px; text-align: center; margin-top: 15px;">
                ...
                <button onclick="simularPago()" style="background-color: #dc3545; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Desbloquear por $9.99</button>
            </div>
        `;
    }
...
// NUEVA FUNCIÓN: DISEÑO FITNESS ESTÉTICO Y COMPATIBLE EN PC/MÓVIL
function descargarPDF() {
    ...
    // Clonamos la rutina pero eliminamos por completo cualquier botón o alerta premium si existiera
    let rutinaLimpia = document.getElementById('res-rutina').innerHTML;
    const contenedorTemporal = document.createElement('div');
    contenedorTemporal.innerHTML = rutinaLimpia;
    const bloquePremium = contenedorTemporal.querySelector('#bloque-premium-web');
    if (bloquePremium) {
        bloquePremium.remove(); // Borra el anuncio premium del PDF final
    }
    rutinaLimpia = contenedorTemporal.innerHTML;

    // Contenedor con diseño Cyberpunk / Gym Oscuro Estético
    ...
    plantillaPDF.style.backgroundColor = '#0f172a'; // Fondo oscuro premium
    ...
    // Configuración de html2pdf para descarga directa forzando fondo oscuro perfecto
    const opciones = {
        margin:       [10, 10, 10, 10],
    ...
    // Descarga directa e inmediata del archivo
